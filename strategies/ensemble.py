...
class Ensemble(Strategy):
    ...
    def __init__(self, account=None):
        ...
        # Only create an XGB instance inside the ensemble for account B
        # to avoid unintentionally loading XGB models for other accounts.
        # If you want XGB enabled for other accounts, change this condition.
        if str(self.account).upper() == 'B':
            self.xgb_strategy = XgbSignal(account=self.account)
        else:
            self.xgb_strategy = None
    ...
    def on_bar(self, bars: dict) -> list[dict]:
        ...
        xgb_signals = self.xgb_strategy.on_bar(bars) if self.xgb_strategy is not None else []
